...
test('ja 전환: 편집기·설정·지출 모달 렌더가 일본어 카탈로그 값 + 통화 ISO 코드', async ({ page }) => {
  await page.goto('/');
  await page.evaluate(() => {
    window.__test.seed('users/u1', { avatarId:'default', tripOrder:['t1'] });
    window.__test.seed('users/u1/trips/t1', { data: JSON.stringify({ title:'X', travelers:['Me'],
  ...
  // 정적 마크업 + 탭
  await expect(page.locator('#backToMypage')).toHaveText('← マイページ');
  await expect(page.locator('#editTabs .tab[data-tab="schedule"]')).toHaveText('旅程');
  await expect(page.locator('#editTabs .tab[data-tab="materials"]')).toHaveText('資料');

  // 일정 탭 placeholder/버튼
  await expect(page.locator('#daysContainer .day-label')).toHaveAttribute('placeholder', '日程のタイトル');
  await expect(page.locator('#daysContainer .add-item')).toHaveText('+ 予定を追加');
  await expect(page.locator('#daysContainer .tl-place')).toHaveAttribute('placeholder', '場所 / やること');

  // 지출 탭 합계 pill
  await page.locator('#editTabs .tab[data-tab="expense"]').click();
  await expect(page.locator('#editView-expense .sum-cur').first()).toHaveText(/^合計（/);

  // 지출 모달: #mCurrency 옵션이 ISO 코드 (JPY 존재, 한글 심볼 '엔' 없음)
  await page.evaluate(() => openExpenseModal('d1', 'i1'));
  await expect(page.locator('#saveExpenseBtn')).toHaveText('＋ 追加');
  const opts = await page.evaluate(() =>
    [...document.querySelectorAll('#mCurrency option')].map(o => o.textContent));
  expect(opts).toContain('JPY');
  expect(opts).toContain('その他（手入力）');
  expect(opts).not.toContain('엔(JPY)');
  await page.evaluate(() => closeExpenseModal());

  // 설정 화면 + 언어 피커 행이 日本語
  await page.evaluate(() => { renderSettings(); showScreen('settings'); });
  await expect(page.locator('#setLang [data-i18n]')).toHaveText('言語');
  await expect(page.locator('#setLangVal')).toHaveText('日本語');
  await expect(page.locator('#setTheme span[data-i18n]')).toHaveText('カラーテーマ');

  await page.evaluate(() => { setLang('ko'); curLang = 'ko'; });
});

test('ja 스모크: 편집기 탭 4종 + 지출 모달 + 설정 + 테마 모달에 한글 없음', async ({ page }) => {
  await page.goto('/');
  await page.evaluate(() => {
    window.__test.seed('users/u1', { avatarId:'default', tripOrder:['t1'] });
    window.__test.seed('users/u1/trips/t1', { data: JSON.stringify({ title:'X', travelers:['Me'],
