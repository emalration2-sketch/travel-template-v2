...
test('문서 없으면 기본값', async ({ page }) => {
  ...
  expect(p).toEqual({ avatarId: 'default', tripOrder: [], theme: 'a', lang: 'ko', wingingDate: '', wingingPicks: [] });
});
...
test('saveProfile 후 재로드 round-trip', async ({ page }) => {
  ...
  const p = await page.evaluate(() => loadProfile());
  expect(p).toEqual({ avatarId: 'fox', tripOrder: ['t1', 't2'], theme: 'a', lang: 'ko', wingingDate: '', wingingPicks: [] });
});
